...
int main(void)
{
	...
	double bestScore = personalTotal[0];
	double worstScore = personalTotal[0];

	for (i = 0; i < STUDENTS; i++)
	{
		if (personalTotal[i] > bestScore)
		{
			bestScore = personalTotal[i];
			bestStudent = i;
		}
		if (personalTotal[i] < worstScore)
		{
			worstScore = personalTotal[i];
			worstStudent = i;
		}
	}
